...
### Fixed
- **Security boundary hardening.** Apricity now binds to `127.0.0.1` by default and refuses unauthenticated non-loopback binds; authentication fails closed when credentials are missing or malformed; token-only deployments require the token on every API request; first-run enrollment requires a generated setup secret; Host, Origin, JSON content type, and per-session CSRF checks protect browser requests; login attempts are throttled; `/api/sync` explicit paths are confined to `MEMPALACE_SYNC_ROOTS`; markdown links reject unsafe schemes; responses carry a restrictive CSP and related security headers; session cookies support `Secure`; the GitHub update check can be disabled; and restore no longer performs a direct SQLite metadata update. Reported by [@XTYZT](https://github.com/XTYZT) in [#17](https://github.com/epinethrone/apricity/issues/17).
