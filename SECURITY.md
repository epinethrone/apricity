...
Apricity is designed to run on **a single trusted machine**, bound to `127.0.0.1`, against a MemPalace install owned by the same user. That assumption shapes everything below.
...
Even though Apricity is local-first, operators can tighten things further:
